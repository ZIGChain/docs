...
export const REDIRECT_ENTRIES: RedirectEntry[] = [
  ...bulk([
    ...
    ["/general/governance", "/users/governance/"],
    ...
    ["/general/staking-apr", "/users/staking/staking-apr"],
    [
...
/** v2 path → old path (v0/v1). Used when switching from v2 to v0/v1 in the version dropdown. */
function trimTrailingSlash(p: string): string {
  return p.endsWith("/") && p.length > 1 ? p.slice(0, -1) : p;
}
export const REVERSE_REDIRECT_MAP: Record<string, string> = (() => {
  const out: Record<string, string> = {};
  for (const { from, to } of REDIRECT_ENTRIES) {
    const key = trimTrailingSlash(to);
    if (!(key in out)) out[key] = from;
  }
  return out;
})();

/** v2 path → all old path variants (v0 and v1 may use different paths for same v2 page). */
export const REVERSE_REDIRECT_MAP_LIST: Record<string, string[]> = (() => {
  const out: Record<string, string[]> = {};
  for (const { from, to } of REDIRECT_ENTRIES) {
    const key = trimTrailingSlash(to);
    if (!out[key]) out[key] = [];
    if (!out[key].includes(from)) out[key].push(from);
  }
  return out;
})();
