...
// Google Analytics 4 (GA4) Measurement ID. Safe to commit — it is public (sent to the browser).
// Get it from https://analytics.google.com/ → Admin → Data Streams → your stream. Set to "" to disable.
const GOOGLE_ANALYTICS_ID = "G-XQ1NBSXMGJ";
...
const config: Config = {
  ...
  plugins: [
    [
      ...
      {
        redirects: Object.entries(REDIRECT_MAP).map(([from, to]) => ({
          from,
          to,
        })),
      },
    ...
    // Tutorials section disabled for now — uncomment to re-enable
    // [
    //   "@docusaurus/plugin-content-docs",
    //   {
    //     id: "tutorials",
    ...
    //     sidebarPath: require.resolve("./sidebars/tutorialsSidebar.ts"),
    //   },
    // ],
  ],
