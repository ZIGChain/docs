...
# Set Governance Voting Period (5 minutes)
echo "NOTE: Setting Governance Voting Period to 5 minutes and 2 minute for expedited requests for easy testing"
jq '.app_state.gov.params.voting_period = "300s"' $HOME/.zigchain/config/genesis.json > $HOME/.zigchain/config/tmp_genesis.json \
&& mv $HOME/.zigchain/config/tmp_genesis.json $HOME/.zigchain/config/genesis.json

jq '.app_state.gov.params.expedited_voting_period = "120s"' $HOME/.zigchain/config/genesis.json > $HOME/.zigchain/config/tmp_genesis.json \
&& mv $HOME/.zigchain/config/tmp_genesis.json $HOME/.zigchain/config/genesis.json

# Add minimum fee for transactions to app.toml
# As it is .toml file we need to use awk to replace the value instead of jq
perl -pi -e 's/^minimum-gas-prices.*/minimum-gas-prices = "0.0025uzig"/' $HOME/.zigchain/config/app.toml
