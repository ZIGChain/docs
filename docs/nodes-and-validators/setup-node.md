---
title: Set Up a ZIGChain Node
description: Step-by-step guide to setting up a ZIGChain full node including hardware requirements, initialization, configuration, system management, and network connectivity.
keywords:
  [
    ZIGChain node setup,
    ZIGChain full node,
    ZIGChain RPC node,
    run ZIGChain node,
    ZIGChain node hardware requirements,
    ZIGChain node configuration,
    ZIGChain full-node sync,
    ZIGChain node data directory,
    ZIGChain node setup guide,
    ZIGChain node setup tutorial,
    ZIGChain full node setup,
  ]
sidebar_position: 4
---
...
:::info
State sync and chain **snapshots** are different ways to catch up quickly. For nodes that serve explorers, dApps, or other RPC clients that require the full historical transaction log, state sync is not appropriate — these workloads still need a fully synced node with the entire block history. If state sync is unavailable or fails, use [Snapshots](https://snapshots.zigchain.com/) as an alternative bootstrap method.
:::
...
You can use our automated setup scripts to quickly install and configure your ZIGChain node, or install it manually by following the [steps below](#manual-setup).
...
- Setting up your node for Mainnet or Testnet
...
Ensure that `zigchain CLI` or `zigchaind` is installed on your system. If not, follow this [installation guide](../builders/quick-start).
...
You can adjust multiple settings as needed. For more details, refer to the [CometBFT Configuration](https://docs.cometbft.com/v0.38/core/configuration).

#### Set Up State Sync

To sync up your node with the network, enable the State Sync service in the `config.toml`.
For the complete, platform-specific walkthrough and troubleshooting tips, refer to the dedicated [Configure State Sync](state-sync) guide.
...
State sync depends on the RPC endpoint still having the trusted block available. If the node fails to sync or the trust window has expired, you can bootstrap from a snapshot instead. See [Snapshots](https://snapshots.zigchain.com/) for available options.
...
<Tabs groupId="peers">
  <TabItem value="Mainnet" label="Mainnet" default>
    ...
    ```toml
    persistent_peers = "782836bc3631ccece23edb9aba62d927956f7c5c@seed0.zigchain.com:26656,79b082fc208a8cd6baa0937490d15d587dc41484@seed1.zigchain.com:26656"
    ```
