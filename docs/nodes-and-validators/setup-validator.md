...
keywords:
  [
    ...
    ZIGChain validator creation steps,
    ZIGChain validator commission,
    ZIGChain commission rate,
    ZIGChain min-self-delegation,
    ZIGChain self-delegation,
    ZIGChain validator node guide,
    run ZIGChain node,
  ]
sidebar_position: 5
---
...
- **Storage:** 500 GB – 1 TB SSD
  - (Exact size depends on pruning and indexing configuration)
...
Configure `timeout_commit` in `~/.zigchaind/config/config.toml` before creating your validator. Per [CometBFT documentation](https://docs.cometbft.com/), all validators must use the same `timeout_commit` value to ensure consistent block production timing and avoid race conditions in optimistic execution.
...
❗️ Mismatched values across validators will cause optimistic execution errors and network performance issues.

To update the config and restart the node:

```bash
sed -i 's/timeout_commit = "5s"/timeout_commit = "2.5s"/' ~/.zigchaind/config/config.toml
systemctl restart zigchaind
```
...
Learn more about fees in the [fees article](../about-zigchain/fees).
...
The **Validator Operator Address** begins with `zigvaloper1` (for more details, check [Validator's Accounts](../about-zigchain/accounts#account-address-format)).
...
- `status:` the validator's status (0: Unspecified, 1: Unbonded, 2: Unbonding, 3: Bonded; for more details check [Validators Statuses](./validators-faq#validator-statuses)).
...
> ⚠️ _If you haven't restored your account yet, you'll need to do so first. See the [CLI command to recover an account](../about-zigchain/accounts#-cli-command-to-recover-an-account) guide._
...
- [Validator Quick Sheet](./validators-quick-sheet)
