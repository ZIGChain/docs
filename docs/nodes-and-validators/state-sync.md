...
:::info
For nodes that serve explorers, dApps, or other RPC clients that require the full historical transaction log, state sync is not appropriate—these workloads still need a fully synced node with the entire block history. If state sync is unavailable or fails, use [Snapshots](https://snapshots.zigchain.com/) as an alternative bootstrap method.
:::
