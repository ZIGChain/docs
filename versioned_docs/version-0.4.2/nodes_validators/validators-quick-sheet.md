...
```sh
zigchaind query staking validator $VALIDATOR_OP_ADDRESS --chain-id="zig-test-1"
```
...
```sh
zigchaind query tendermint-validator-set --chain-id="zig-test-1" | grep "$(zigchaind tendermint show-address)"
```
...
```sh
zigchaind query slashing signing-info $VALIDATOR_CON_ADDRESS --chain-id="zig-test-1"
...
```sh
zigchaind tx slashing unjail --chain-id="zig-test-1" \
  --gas="auto" \
  --gas-prices="0.00025uzig" \
  --from=zuser1 \
  --gas-adjustment=1.5
...
```sh
zigchaind query staking validator --chain-id="zig-test-1"
```
...
```sh
zigchaind query distribution commission $VALIDATOR_OP_ADDRESS --chain-id="zig-test-1"
```
...
```sh
zigchaind query distribution validator-distribution-info $VALIDATOR_OP_ADDRESS --chain-id="zig-test-1"
```
